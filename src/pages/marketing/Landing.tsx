...
export default function Landing() {
  ...
  const pillClass =
    "fixed bottom-6 right-6 md:bottom-8 md:right-8 z-50 group inline-flex items-center gap-2 h-11 pl-5 pr-4 rounded-full bg-primary text-primary-foreground text-[12.5px] uppercase tracking-[0.18em] font-medium border hairline shadow-paper hover:translate-y-[-1px] transition-transform";
  ...
  return (
    <main className="min-h-screen bg-background text-foreground bg-paper">
      ...
      {user ? (
        <Link to="/app" aria-label="Open Studio" className={pillClass}>
        ...
        <button type="button" onClick={() => setSignInOpen(true)} aria-label="Sign in" className={pillClass}>
          <span>Sign in</span>
          <ArrowUpRight className="h-3.5 w-3.5 transition-transform group-hover:translate-x-0.5 group-hover:-translate-y-0.5" strokeWidth={2} />
        </button>
      )}
